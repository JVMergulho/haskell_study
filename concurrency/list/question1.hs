...
-- As funções recebem um MVar extra para sincronizar a saída. Isso é necessário para que a saída não seja misturada entre as threads.

import Control.Concurrent
import Control.Monad

fabrica:: MVar Int -> String -> MVar() -> IO()
fabrica caixa item lock = forever $ do
...
montaPar caixaPorcas caixaParafusos caixaPares lock = forever $ do
            threadDelay 500000 -- delay de 0.5s
